...
const createCustomAchievement = async (req, res) => {
  try {
    if (!req.session.userId) {
      return res.status(401).json({ message: 'No autenticado' });
    }

    const { name, description, icon, points, targetValue, metric } = req.body;
    ...
    const newAchievement = {
      ...
      metric: metric || 'habits_completed',
      currentValue: 0,
      unlocked: false
    };

    user.customAchievements.push(newAchievement);
    await user.save();

    res.status(201).json({ 
      message: 'Logro personalizado creado', 
      achievement: user.customAchievements[user.customAchievements.length - 1] 
    });
  } catch (error) {
    res.status(500).json({ message: 'Error al crear logro personalizado', error: error.message });
  }
};

// Obtener logros personalizados del usuario
const getCustomAchievements = async (req, res) => {
  try {
    if (!req.session.userId) {
      return res.status(401).json({ message: 'No autenticado' });
    }
    ...
    res.json({ customAchievements: user.customAchievements });
  } catch (error) {
    res.status(500).json({ message: 'Error al obtener logros personalizados', error: error.message });
  }
};

// Actualizar logro personalizado
const updateCustomAchievement = async (req, res) => {
  try {
    if (!req.session.userId) {
      return res.status(401).json({ message: 'No autenticado' });
    }
    ...
    const achievement = user.customAchievements.id(achievementId);
    if (!achievement) {
      return res.status(404).json({ message: 'Logro personalizado no encontrado' });
    }
    ...
    if (targetValue) achievement.targetValue = targetValue;
    if (metric) achievement.metric = metric;

    await user.save();

    res.json({ message: 'Logro personalizado actualizado', achievement });
  } catch (error) {
    res.status(500).json({ message: 'Error al actualizar logro personalizado', error: error.message });
  }
};

// Eliminar logro personalizado
const deleteCustomAchievement = async (req, res) => {
  try {
    if (!req.session.userId) {
      return res.status(401).json({ message: 'No autenticado' });
    }
    ...
    user.customAchievements = user.customAchievements.filter(
      a => a.id !== achievementId
    );

    await user.save();

    res.json({ message: 'Logro personalizado eliminado' });
  } catch (error) {
    res.status(500).json({ message: 'Error al eliminar logro personalizado', error: error.message });
  }
};

// Marcar logro personalizado como completado manualmente
const unlockCustomAchievement = async (req, res) => {
  try {
    if (!req.session.userId) {
      return res.status(401).json({ message: 'No autenticado' });
    }
    ...
    if (achievement.unlocked) {
      return res.status(400).json({ message: 'Este logro ya está desbloqueado' });
    }
    ...
    res.json({ 
      message: 'Logro personalizado desbloqueado', 
      achievement,
      newPoints: user.points,
      newLevel: user.level
    });
  } catch (error) {
    res.status(500).json({ message: 'Error al desbloquear logro personalizado', error: error.message });
  }
};
